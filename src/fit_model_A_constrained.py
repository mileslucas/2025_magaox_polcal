...
def model(X, X2, hwp_deg, imr_deg, S_in):
    """
    Model A is
...
def fit_model_scipy(table_lp0, table_nolp, prior, name: str):
    ...
    def loss(X):
        # # check V+S == extinction
        # pbs_extinction = X2[-1]
        # pbs_V_diatt = X[-2]
        # pbs_H_diatt = X[-1]
        # _pbs_ext = (pbs_V_diatt - pbs_H_diatt) / (pbs_V_diatt + pbs_H_diatt)
        # if np.abs(_pbs_ext - pbs_extinction) > 0.1:
        #     return np.inf

        input_S = np.array([1, 1, 0, 0])
        y_hat = np.array(
            [model(X, X2, h, i, input_S) for h, i in zip(hwp_angs_lp0, imr_angs_lp0)]
        )
        ll_lp0 = log_likelihood(y_hat, y_values_lp0, y_err_lp0)

        input_S = np.array([1, 0, 0, 0])
        y_hat = np.array(
            [model(X, X2, h, i, input_S) for h, i in zip(hwp_angs_nolp, imr_angs_nolp)]
        )
        ll_nolp = log_likelihood(y_hat, y_values_nolp, y_err_nolp)
        # print(f"{ll_lp0=} {ll_nolp=}")
